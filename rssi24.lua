...
----------------------------------------------------------------------------------------------------------------------
local name = "RSSI 2.4 min/current"
local key = "rssi24"
...
local function paint(widget)
    ...
    local displayString = "---/---"
    if widget ~= nil then
        displayString = math.floor(widget.service.rssi_24) .. "/" .. math.floor(widget.vrf_current)
    end
...
local function wakeup(widget)
    ...
    widget.vrf_current = system.getSource("RSSI"):value()
    if widget.service.rssi_24 < widget.vrf_current then
        widget.service.rssi_24 = widget.vrf_current
    end
end
