...
----------------------------------------------------------------------------------------------------------------------
local name = "VFR 2.4 min/current"
local key = "vfr24"
...
local function paint(widget)
    ...
    local displayString = "---/---"
    if widget ~= nil then
        displayString = math.floor(widget.service.vfr_24) .. "/" .. math.floor(widget.vrf_current)
    end
...
local function wakeup(widget)
    ...
    widget.vrf_current = system.getSource("VFR"):value()
    if widget.service.vfr_24 < widget.vrf_current then
        widget.service.vfr_24 = widget.vrf_current
    end
end
